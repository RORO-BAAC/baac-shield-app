...
const [qaqcDuctVarianceDetails, setQaqcDuctVarianceDetails] = useState("");
 ...
 const [qaqcHandholeVaultDocumentationComplete, setQaqcHandholeVaultDocumentationComplete] = useState("");  
    const [qaqcHandholeGroundingBondingVerified, setQaqcHandholeGroundingBondingVerified] = useState("");  
      const [qaqcHandholeVaultLabelingComplete, setQaqcHandholeVaultLabelingComplete] = useState("");
...
const [qaqcCablePullTensionVerified, setQaqcCablePullTensionVerified] = useState("");
   const [qaqcCableEndsProtected, setQaqcCableEndsProtected] = useState("");   
    const [qaqcCableIdentificationVerified, setQaqcCableIdentificationVerified] = useState("");  
...
const [qaqcFibreTestCalibrationCurrent, setQaqcFibreTestCalibrationCurrent] = useState("");
 const [qaqcFibreTestReportReferenced, setQaqcFibreTestReportReferenced] = useState("");   
  const [qaqcFibreTestBidirectionalComplete, setQaqcFibreTestBidirectionalComplete] = useState("");    
...
const [alertEmail, setAlertEmail] = useState("yycrgonzalez@gmail.com");
...
if (!vehiclePreUseDriverSignature) {
  setMessage("The driver must sign the vehicle inspection.");
  return;
}
...
function downloadQaqcInspectionPdf(inspection) {
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(18);
  doc.text(`${companyName} QA/QC WORK INSPECTION`, 14, 18);
  ...
  addLine("Worksite", inspection.inspection_location);
  addLine("Work Type", inspection.work_type);
  addLine("Contractor / Crew", inspection.contractor_crew);
  ...
  addLine("Work Inspected", inspection.work_inspected);
...
addLine("Utility Clearance Verified", inspection.utility_clearance_verified);
      ...
      addLine(
  "Final Handhole / Vault Documentation Complete",
  ...
  inspection.handhole_grounding_bonding_verified
    ...
    addLine(
  "Handhole / Vault Labeling & Conduit Tags Complete",
  inspection.handhole_vault_labeling_complete
);    
...
function downloadQaqcCableInspectionPdf(inspection) {
  if (!inspection) return;
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(17);
  doc.text(`${companyName} QA/QC CABLE PLACEMENT INSPECTION`, 14, 18);
  ...
  addLine("Cable Run / Segment", inspection.cable_run_id);
  addLine("Cable Type", inspection.cable_type);
  addLine("Cable Size / Fibre Count", inspection.cable_size_count);
...
addLine(
  ...
  addLine("Splice Loss / Test Results Acceptable", record.splice_loss_acceptable);          
  ...
  doc.save(`qaqc-fibre-splicing-${record.id}.pdf`);
      ...
      function downloadQaqcFibreTestPdf(record) {
  if (!record) return;
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(17);
  doc.text(`${companyName} QA/QC FIBRE TESTING RECORD`, 14, 18);
  ...
  addLine("Inspection Date", record.inspection_date);
  addLine("Technician", record.technician_name);
  addLine("Cable ID", record.cable_id);
  ...
  doc.save(`qaqc-fibre-testing-${record.id}.pdf`);
}
  function downloadQaqcEquipmentPdf(record) {
  if (!record) return;
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(17);
  doc.text(`${companyName} QA/QC EQUIPMENT INSTALLATION RECORD`, 14, 18);
  ...
  addLine("Project", projectName);
  addLine("Client / Owner", record.client_owner);
  addLine("Location", record.installation_location);
  addLine("Installation Date", record.installation_date);
  addLine("Installer / Subcontractor", record.installer_subcontractor);
  addLine("Equipment Description", record.equipment_description);
  addLine("Manufacturer", record.manufacturer);
  addLine("Model", record.model);
  addLine("Serial Number", record.serial_number);
  ...
  doc.save(`qaqc-equipment-installation-${record.id}.pdf`);
}    
      ...
      .insert([
        {
          ...
          tracer_wire_installed: qaqcTracerWireInstalled,
              tracer_wire_continuity_verified: qaqcTracerWireContinuityVerified,
...
variance_details: qaqcDuctVarianceDetails,
             utility_clearance_verified: qaqcUtilityClearanceVerified, 
              ...
              restoration_complete: qaqcRestorationComplete,
              handhole_vault_documentation_complete: qaqcHandholeVaultDocumentationComplete,
              handhole_grounding_bonding_verified: qaqcHandholeGroundingBondingVerified,
              handhole_vault_labeling_complete: qaqcHandholeVaultLabelingComplete,
...
spec_verified: qaqcCableSpecVerified,
          ...
          pathway_used: qaqcCablePathwayUsed,
              route_verified: qaqcCableRouteVerified,
...
pull_tension_verified: qaqcCablePullTensionVerified,
            cable_ends_protected: qaqcCableEndsProtected,  
              cable_identification_verified: qaqcCableIdentificationVerified,
...
calibration_current: qaqcFibreTestCalibrationCurrent,
           report_referenced: qaqcFibreTestReportReferenced,   
            bidirectional_complete: qaqcFibreTestBidirectionalComplete,  
    ...
    setQaqcFibreTestFibreCount("");
     setQaqcFibreTestType("");   
        setQaqcFibreTestWavelength("");
        setQaqcFibreTestEquipment("");
        setQaqcFibreTestEquipmentSerial("");
    setQaqcFibreTestCalibrationCurrent("");   
     setQaqcFibreTestReportReferenced("");   
     setQaqcFibreTestBidirectionalComplete("");   
...
{[
  {
   title: "Vehicle Check",
    items: [
      ...
      ["tiresCondition", "Tires and wheels"],
      ["headlights", "Lights and signals"],
      ["windshield", "Windshield, mirrors and wipers"],
      ["dashboardWarningLights", "Dashboard warning indicators"],
      ["serviceBrakes", "Brakes and steering"],
      ["visibleFluidLeaks", "Visible leaks or damage"],
      ["registrationPresent", "Registration and insurance"],
...
>
 ...
 <section>
  ...
  .sort(
    (a, b) =>
      new Date(b.flra_date || b.created_at || 0) -
      new Date(a.flra_date || a.created_at || 0)
  )
  .slice(0, 10)
  .map((item) => (
          <div
 ...
 {openRecordsSection === "qaqc-duct-pathway" && (
     <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
        ...
        .map((inspection) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ marginTop: 4, fontSize: 13, color: "#475569" }}>
                Client / Owner: {inspection.client_owner || "-"} | Location:{" "}
                {inspection.inspection_location || "-"}
              </div>

              <div style={{ marginTop: 4, fontSize: 13, color: "#475569" }}>
                Duct Run: {inspection.duct_run_id || "-"} | Inspector:{" "}
                {inspection.inspector_name || "-"}
              </div>

              <div style={{ marginTop: 4, fontSize: 13, color: "#475569" }}>
                Result: {inspection.inspection_result || "-"} | Status:{" "}
                {inspection.inspection_status || "-"}
              </div>

              <button
                type="button"
                onClick={() => downloadQaqcDuctInspectionPdf(inspection)}
                style={{
                  marginTop: 10,
                  padding: "8px 12px",
                  borderRadius: 10,
      ...
      {qaqcDuctInspections.filter((inspection) => {
        ...
        <div style={{ color: "#64748b" }}>
          No QA/QC duct / pathway records match the selected filters.
        </div>
      )}
  ...
  <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
      ...
      .filter((inspection) => {
        ...
        const matchesSearch =
          ...
          inspection.client_owner?.toLowerCase().includes(q) ||
          inspection.inspection_location?.toLowerCase().includes(q) ||
          inspection.cable_run_id?.toLowerCase().includes(q) ||
      ...
      .map((inspection) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ marginTop: 4, fontSize: 13, color: "#475569" }}>
              Cable Run: {inspection.cable_run_id || "-"} | Cable Type:{" "}
              {inspection.cable_type || "-"}
            </div>
            ...
            <button
              type="button"
              onClick={() => downloadQaqcCableInspectionPdf(inspection)}
              style={{
                marginTop: 10,
                padding: "8px 12px",
                borderRadius: 10,
    ...
    {qaqcCableInspections.filter((inspection) => {
      const q = recordsCenterSearch.trim().toLowerCase();
...
>
  <button
    ...
    onClick={() =>
      setOpenRecordsSection(
        openRecordsSection === "qaqc-splicing" ? "" : "qaqc-splicing"
      )
    }
    style={{
      width: "100%",
  ...
  >
    {openRecordsSection === "qaqc-splicing" ? "▼" : "▶"} QA/QC Fibre Splicing Records ({qaqcSpliceRecords.length})
  </button>
       {openRecordsSection === "qaqc-splicing" && (
  <div style={{ marginTop: 16, display: "grid", gap: 12 }}>
    {qaqcSpliceRecords.map((record) => (
      ...
      >
        ...
        <div style={{ marginTop: 6 }}>
          <strong>Date:</strong> {record.splice_date || "—"}
        </div>

        <div>
          <strong>Location:</strong> {record.splice_location || "—"}
        </div>
        ...
        <div>
          <strong>Closure ID:</strong> {record.closure_id || "—"}
        </div>
...
<div>
  <strong>Test Type:</strong> {record.test_type || "—"}
</div>
              <div>
  ...
  <strong>Acceptance Criteria Met:</strong> {record.acceptance_criteria_met || "—"}
        ...
        <div>
          <strong>Status:</strong> {record.test_report_status || "—"}
             ...
             <button
  type="button"
  onClick={() => downloadQaqcFibreTestPdf(record)}
  style={{
    marginTop: 10,
    padding: "8px 12px",
    borderRadius: 10,
...
>
  ...
  <div style={{ marginTop: 16, display: "grid", gap: 12 }}>
    {qaqcCloseoutRecords.map((record) => (
      <div
        key={record.id}
        style={{
          padding: 14,
      ...
      >
       ...
       {allProjects.find((p) => String(p.id) === String(record.project_id))?.name ||
        ...
        <div style={{ marginTop: 6 }}>
          <strong>Date:</strong> {record.closeout_date || "—"}
        </div>

        <div>
          <strong>Location:</strong> {record.closeout_location || "—"}
        </div>

        <div>
          <strong>Client / Owner:</strong> {record.client_owner || "—"}
        </div>
...
{activeTab === "qaqc" &&
  qaqcSection === "telecom" &&
  qaqcInspectionType === "duct-pathway-records" && (
    <div
      style={{
        display: "grid",
        gap: 18,
...
>
  ...
  {selectedQaqcDuctInspection.installation_method || "-"}
    ...
    <p><strong>Tracer Wire Installed:</strong> {selectedQaqcDuctInspection.tracer_wire_installed || "-"}</p>
      <p><strong>Tracer Wire Continuity / Locate Verification:</strong> {selectedQaqcDuctInspection.tracer_wire_continuity_verified || "-"}</p>    
...
>
  ...
  <select
    value={qaqcSplicePlanVerified}
    onChange={(e) => setQaqcSplicePlanVerified(e.target.value)}
    style={{
      padding: 11,
      borderRadius: 10,
      border: "1px solid #cbd5e1",
...
{activeTab === "qaqc" &&
  qaqcSection === "telecom" &&
  qaqcInspectionType === "splicing-records" && (
    <div
      style={{
        display: "grid",
        gap: 18,
...
>
  ...
  {selectedQaqcSpliceRecord.project_id || "-"}
      ...
      <div><strong>Splice Loss / Test Results Acceptable:</strong><br />{selectedQaqcSpliceRecord.splice_loss_acceptable || "-"}</div>
...
{activeTab === "qaqc" &&
  qaqcSection === "telecom" &&
  qaqcInspectionType === "fibre-testing" && (
    <div
      style={{
        display: "grid",
        gap: 18,
...
>
  ...
  >
    ...
    <option value="N/A">N/A</option>
       ...
       <label>
  <strong>OTDR / OLTS Test Files or Report Attached / Referenced?</strong>
  <select
    value={qaqcFibreTestReportReferenced}
    onChange={(e) => setQaqcFibreTestReportReferenced(e.target.value)}
    style={{
      width: "100%",
      padding: 10,
      marginTop: 6,
  ...
  >
    ...
    <label>
  <strong>Bi-Directional Testing Completed?</strong>
  <select
    value={qaqcFibreTestBidirectionalComplete}
    onChange={(e) => setQaqcFibreTestBidirectionalComplete(e.target.value)}
    style={{
      width: "100%",
      padding: 10,
      marginTop: 6,
...
{activeTab === "qaqc" &&
  qaqcSection === "telecom" &&
  qaqcInspectionType === "equipment-installation" && (
    <div
      style={{
        display: "grid",
        gap: 18,
...
<label>
  Equipment Mounted / Secured Correctly?
  <select
    value={qaqcEquipmentMountedSecure}
    onChange={(e) => setQaqcEquipmentMountedSecure(e.target.value)}
    style={{
      width: "100%",
      marginTop: 6,
      padding: 10,
  ...
  <label>
   <label>
  Grounding / Bonding Complete?
  <select
    value={qaqcEquipmentGroundingBondingComplete}
    onChange={(e) =>
      setQaqcEquipmentGroundingBondingComplete(e.target.value)
    }
    style={{
      width: "100%",
      marginTop: 6,
...
>
 <label>
  ...
  >
    ...
    <label>
  Compaction / Backfill Verified?
  <select
    value={qaqcCompactionBackfillVerified}
    onChange={(e) => setQaqcCompactionBackfillVerified(e.target.value)}
    style={{
      width: "100%",
      padding: 10,
      borderRadius: 10,
  ...
  Final Handhole / Vault Documentation Complete?
  <select
    value={qaqcHandholeVaultDocumentationComplete}
    onChange={(e) => setQaqcHandholeVaultDocumentationComplete(e.target.value)}
    style={{
      width: "100%",
      padding: 10,
      borderRadius: 10,
...
<label>
  ...
  Handhole / Vault Labeling & Conduit Tags Complete?
  <select
    value={qaqcHandholeVaultLabelingComplete}
    onChange={(e) => setQaqcHandholeVaultLabelingComplete(e.target.value)}
    style={{
      width: "100%",
      padding: 10,
      borderRadius: 10,
